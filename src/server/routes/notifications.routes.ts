...
export const notificationRoutes = () => {
	...
	app.get("/notifications", requireRole("admin"), (c) => {
	...
	app.post("/notifications/:id/read", requireRole("admin"), (c) => {
		const id = Number(c.req.param("id"));
		markNotificationRead.run(id);
		return c.json({ ok: true });
	});

	app.post("/notifications/read-all", requireRole("admin"), (c) => {
		const user = c.var.user!;
