<script lang="ts">
  ...
  let {
    ...
    templates,
    webhookUrl,
  }: { whatsapp: { provider: string; apiKey: string; adminNotifyNumber: string; integrationUrl: string }; templates: WhatsAppTemplate[]; webhookUrl: string } =
    $props()
  ...
  // Reveal the API key (password-style show/hide).
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <Field id="wc-key" label="API key">
                  <div class="relative">
                    ...
                    <button
                      type="button"
                      class="absolute inset-y-0 right-0 flex items-center pr-3 text-muted hover:text-text"
                  ...
                  <p class="text-xs text-muted mt-1">
                    {cfgForm.api_key
                      ? 'A key is set. Type a new one to replace it, or leave unchanged to keep it.'
