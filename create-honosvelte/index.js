...
const REPO = 'kabarpos/honosvelte'

/** Templates — each maps to a git branch in the Honosvelte repo. */
const TEMPLATES = [
  {
    name: 'default',
...
function help() {
  console.log(`
${c.bold('create-honosvelte')} — scaffold a new Honosvelte project

${c.bold('Usage:')}
  ${c.cyan('bunx create-honosvelte')} ${c.dim('<project-name>')}
  ${c.cyan('bunx create-honosvelte')} ${c.dim('.')}   ${c.dim('# use current directory')}
...
${c.bold('Examples:')}
  ${c.cyan('bunx create-honosvelte')} my-app
  ${c.cyan('bunx create-honosvelte')} my-app --template svelte-tailwind
  ${c.cyan('bunx create-honosvelte')} my-app --no-install
`)
}
...
async function main() {
  ...
  console.log(`${c.cyan('↓')} Downloading Honosvelte (${template.label})...`)
  try {
  ...
  await Promise.all(
    CLEANUP.map((p) => rm(join(targetDir, p), { recursive: true, force: true })),
  )
  ...
  console.log()
  console.log(c.green('✓ Honosvelte project created!'))
  ...
  console.log()
  console.log(c.dim('Docs: https://github.com/kabarpos/honosvelte'))
}
